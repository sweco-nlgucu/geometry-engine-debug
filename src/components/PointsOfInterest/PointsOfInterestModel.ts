...
> {
    ...
    async createNew(location: HasGeometry): Promise<void> {
        ...
        geometryEngineAsync.distance(poi.geometry , poi.geometry , "meters")
        .then((distance) => {
            console.log(`The geometryEngineAsync works! Distance should be roughly 0: ${distance}`);
            this.pointsOfInterest.add(poi);

        })
        .catch((err) => console.error("geometryEngineAsync error, but at least it called!", err));
    }
